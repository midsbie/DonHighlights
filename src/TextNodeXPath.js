...
allowed.  Offending XPath representation: ${xpath}`
  ...
  /**
   * Calculate the relative offset from a specified text node (`node`) to the start of the first
   * **sibling** text node in a set of contiguous text or highlight container nodes.
   *
   * For example, given the post-highlight, non-normalised, child contents of an arbitrary element:
   *
   *   `#text + SPAN.hh-highlight + #text + STRONG`
   *
   * If this method were invoked with `node` containing a reference to the third `#text` element,
   * the computed offset would be the length of the `SPAN` to its left plus the length of the first
   * `#text`.  This because the normalised version -- pre-highlight, that is -- of the above would
   * be:
   *
   *   `#text + STRONG`
   *
  ...
  offset(node: Node): number {
    ...
    while (true) {
      while ((node: any).previousSibling == null) {
        ...
        } else if (!this._isHighlight(node)) {
          return offset;
   ...
   *
   * Note that no checks are made to ensure that the node is either a highlight container or text
   * node.  Caller is responsible for invoking this method in the right context.
   *
   * @access private
  ...
  skip_(node: Node): Node {
    ...
    if (node.nodeType === 3) {
      while (node != null) {
        ...
        if (parent === this.root || !this._isHighlight(parent)) {
          break;
        }
   ...
   */
  _isHighlight(node: Node): boolean {
    ...
    return node.nodeType === 1 && (node: any).dataset.domHighlight === 'true';
  }
  ...
  isLikeText_(node: Node): boolean {
    return node.nodeType === 3 || this._isHighlight(node);
  }
  ...
  nthElementOf_(parent: Node, tag: string, index: number): Node | null {
    ...
    console.error(`failed to locate tag '${tag}' at index ${String(index)}`);
    return null;
  }
  ...
  nthTextOf_(parent: Node, index: number): Node | null {
    ...
    for (let i = 0, l = ch.length; i < l; ++i) {
      ...
      if (index === 0) {
        ...
        if (node.nodeType !== 3) {
          console.error(`failed to locate text node at index ${String(index)}`);
          return null;
        }
